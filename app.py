...
use_case = {
    "1":
    "Image background removal - (upload any picture and remove background)",
...
def init_page():
    st.set_page_config(
        page_title=
        'TWC - Image foreground masking or background removal with state-of-the-art models',
        page_icon="logo.jpg",
        layout='centered',
        initial_sidebar_state='auto',
        menu_items={
            'About':
            'This app was created by taskswithcode. http://taskswithcode.com'
        })
...
def run_test(config, input_file_name, display_area, uploaded_file, mask_type):
    ...
    try:
        if (uploaded_file is None):
            ...
            r = requests.post(config["SERVER_ADDRESS"],
                              data={"mask": mask_type},
...
def app_main(app_mode, example_files, model_name_files, api_info_files,
             config_file):
    init_session()
    ...
    try:
        ...
        with st.form('twc_form'):
            ...
            uploaded_file = st.file_uploader(step1_line,
                                             type=["png", "jpg", "jpeg"])

            selected_file_name = image_select("Select image", [
                "twc_samples/sample1.jpg", "twc_samples/sample2.jpg",
                "twc_samples/sample3.jpg", "twc_samples/sample4.jpg"
            ])
            ...
            if submit_button:
                ...
                display_area.empty()
                results = run_test(config, st.session_state["file_name"],
                                   display_area, uploaded_file, mask_type)
        ...
        st.download_button(
            ...
            if st.session_state["download_ready"] != None else "",
            disabled=False
            if st.session_state["download_ready"] != None else True,
            file_name=(
                st.session_state["model_name"] + "_" +
                st.session_state["mask_type"] + "_" +
...
if __name__ == "__main__":
    app_main("1", "sod_app_examples.json", "sod_app_models.json",
             "sod_apis.json", "config.json")
